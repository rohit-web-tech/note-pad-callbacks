...
function saveFile() {
    if (index != null) {
        ...
        if (editor.value != '') {
            myPrompt("Enter file name to save current file.", function (fileName) {
...
function display() {
    ...
    editBtn.forEach((btn) => {
        btn.onclick = () => {
            if (editor.value != '') {
                myConfirm("Do you want to save current file?", function (result) {
                    if (result) {
...
function deleteFile(index1) {
    ...
    myConfirm(`Do you really want to delete ${fileName} file?`, function (result) {
        if (result) {
...
function myPrompt(content, callback) {
...
clearAllBtn.onclick = () => {
    myConfirm("Do you really want to close file ?", function (result) {
        if (result) {
            closeFile();
        }
    });
